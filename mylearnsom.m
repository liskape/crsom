function [out1,out2] = mylearnsom(varargin)
...
function [dw,ls, E, new_cn] = apply(w,p,z,n,a,t,e,gW,gA,d,lp,ls, cn, target, t_epoch, crsom, LR2, s_0, send)
  ...
  [dw, new_cn, E, O_h, first_part, second_part] = context_net_adapt2( cn, crsom, p, target, find(a), t_epoch, crsom.trainParam.epochs, LR2, s_0, send );
    ...
    if crsom.userdata.computing_details
          crsom.userdata.mean_adjusts = [crsom.userdata.mean_adjusts mean(mean(dw))];
        crsom.userdata.mean_O_h = [crsom.userdata.mean_O_h mean(O_h)];
